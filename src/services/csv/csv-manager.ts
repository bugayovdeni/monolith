...
export class CsvManager {
  ...
  async init(): Promise<void> {
    try {
      await listen<CsvLoadedEvent>(
        "csv://loaded",
        (event: Event<CsvLoadedEvent>) => {
  ...
  private async fetchAndProcess(): Promise<void> {
    ...
    try {
      const data = await invoke<CementingData>("get_csv_data", {
        sessionId: this.sessionId,
      });
      console.log(
        `✅ CsvManager: ${data.file_name}, ${data.records.length} записей`,
      );
      ...
      if (
        validPointsCount === 0 ||
        Object.values(bulkData).every((arr) => arr.length === 0)
      ) {
